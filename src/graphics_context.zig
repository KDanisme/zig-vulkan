...
const optional_instance_extensions = &[_]vk.ApiInfo{
    vk.extensions.khr_get_physical_device_properties_2,
};

const features = &[_]vk.ApiInfo{ vk.extensions.khr_get_physical_device_properties_2, vk.extensions.khr_swapchain, vk.extensions.khr_surface, vk.features.version_1_0 };
const BaseDispatch = vk.BaseWrapper(features);

const InstanceDispatch = vk.InstanceWrapper(features);

const DeviceDispatch = vk.DeviceWrapper(features);
...
pub const GraphicsContext = struct {
    ...
    pub fn init(allocator: Allocator, app_name: [*:0]const u8, window: SDL.Window) !GraphicsContext {
        ...
        var instance_extensions: [][*:0]const u8 = undefined;
        const count = try SDL.vulkan.getInstanceExtensions(window, &instance_extensions);
        // defer

        // std.log.info("{s}", .{propsv});
        for (optional_instance_extensions) |extension| {
            for (propsv) |prop| {
                const len = std.mem.indexOfScalar(u8, &prop.extension_name, 0).?;
                const prop_ext_name = prop.extension_name[0..len];
                if (std.mem.eql(u8, prop_ext_name, extension.name)) {
                    try instance_extensions.append(@ptrCast(extension.name));
                    break;
                }
            }
        }
        for (instance_extensions.items) |extension| {
            std.debug.print("{s}\n", .{extension});
        }
        ...
        self.instance = try self.vkb.createInstance(
            &vk.InstanceCreateInfo{ .flags = if (builtin.os.tag == .macos) .{
                .enumerate_portability_bit_khr = true,
            } else .{}, .p_application_info = &app_info, .enabled_layer_count = 0, .pp_enabled_layer_names = undefined, .enabled_extension_count = count, .pp_enabled_extension_names = instance_extensions },
